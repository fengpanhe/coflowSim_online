...
static std::shared_ptr<spdlog::logger> socketManage_logger;
void SocketManage::initSocket(int sockfd, const sockaddr_in &addr) {
  ...
  try {
    ...
    socketManage_logger = spdlog::rotating_logger_mt(
...
//  printf("recvmsg: \"%s\" \n", m_recvBuf);
  socketManage_logger->info("recvmsg: \"{}\" ", m_recvBuf);
  return true;
}
...
bool SocketManage::sendMessage() {
  ...
  if (temp==m_sendBufSize) {
    modfd(SocketManage::sEpollfd, m_Sockfd, EPOLLIN);
//    printf("sendmsg: \"%s\" \n", m_sendBuf);
    socketManage_logger->info("sendmsg: \"{}\" ", m_sendBuf);
    initSendBuf();
    return true;
  }
